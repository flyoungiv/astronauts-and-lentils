...
I brought in as few packages/libraries as possible for this challenge because I wanted to demonstrate that I know the fundamentals. For example, I opted to use the style tag and write css directly in the React jsx files. I am comfortable with css-in-js, styled components, css modules, etc., but I wanted to show that I don't have to depend on those things. I specifically avoided bringing in any UI-frameworks (Material-UI, Semantic-UI) for the same reason.

I took a similar "bare-bones" approach with the backend. I have lots of experience working with enormous Oracle and SQL Server DB's, but I am a big believer in choosing the right tool for the job. For this reason I chose the lowdb package... the data provided was already in a .json format, so I figured why not just lean into that. On a larger scale I would lean on a different kind of setup, like MongoDB/Firebase/some other kind of No-SQL solution, but I liked the way the lowdb package got the job done without a bunch of overhead.
...
- add a fuzzy search-as-you-type input box to automatically bring back results with each keypress. this could be easily done with several different node packages in the back end, especially for a dataset this small. with a larger dataset we would need to consider a more scaleable approach.
